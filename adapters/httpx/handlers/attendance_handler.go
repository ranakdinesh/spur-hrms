...
func (h *Handler) CreateAttendanceWorkdaySegment(w http.ResponseWriter, r *http.Request) {
	tenantID, err := h.tenantIDFromRequest(r)
	if err != nil {
		h.respondError(w, r, http.StatusUnauthorized, "create attendance segment", err, "tenant context is required")
		return
	}
	h.createAttendanceWorkdaySegmentForTenant(w, r, tenantID, "create attendance segment")
}

func (h *Handler) ListAttendanceWorkdaySegments(w http.ResponseWriter, r *http.Request) {
	tenantID, err := h.tenantIDFromRequest(r)
	if err != nil {
		h.respondError(w, r, http.StatusUnauthorized, "list attendance segments", err, "tenant context is required")
		return
	}
	h.listAttendanceWorkdaySegmentsForTenant(w, r, tenantID, "list attendance segments")
}
...
func (h *Handler) CreateTenantAttendanceWorkdaySegment(w http.ResponseWriter, r *http.Request) {
	tenantID, ok := h.superAdminTenantID(w, r, "create tenant attendance segment")
	if ok {
		h.createAttendanceWorkdaySegmentForTenant(w, r, tenantID, "create tenant attendance segment")
	}
}

func (h *Handler) ListTenantAttendanceWorkdaySegments(w http.ResponseWriter, r *http.Request) {
	tenantID, ok := h.superAdminTenantID(w, r, "list tenant attendance segments")
	if ok {
		h.listAttendanceWorkdaySegmentsForTenant(w, r, tenantID, "list tenant attendance segments")
	}
}
...
func (h *Handler) createAttendanceWorkdaySegmentForTenant(w http.ResponseWriter, r *http.Request, tenantID uuid.UUID, operation string) {
	var cmd ports.AttendanceSegmentCommand
	if err := json.NewDecoder(r.Body).Decode(&cmd); err != nil {
		h.respondError(w, r, http.StatusBadRequest, "decode "+operation+" request", err, "invalid request body")
		return
	}
	...
	if !h.requireOwnUserOrPermission(w, r, operation, cmd.UserID,
		[]string{permissions.AttendanceSelfPunch, permissions.AttendanceSelfView, permissions.AttendanceCheckIn, permissions.AttendanceCheckOut},
		[]string{permissions.AttendanceOperationsManage},
	) {
		return
	}
	cmd.ActorID = h.actorIDFromRequest(r)
	item, err := h.svc.CreateAttendanceWorkdaySegment(r.Context(), cmd)
	if err != nil {
		h.respondError(w, r, http.StatusBadRequest, operation, err, err.Error())
		return
	}
...
func (h *Handler) listAttendanceWorkdaySegmentsForTenant(w http.ResponseWriter, r *http.Request, tenantID uuid.UUID, operation string) {
	query := r.URL.Query()
	userRaw := query.Get("user_id")
	var userID uuid.UUID
	if userRaw != "" {
	...
	if date := query.Get("date"); date != "" {
		items, err := h.svc.ListAttendanceWorkdaySegmentsByUserDate(r.Context(), tenantID, userID, date)
		if err != nil {
			h.respondError(w, r, http.StatusInternalServerError, operation, err, "failed to list attendance segments")
			return
		}
		respondJSON(w, http.StatusOK, items)
		return
	}
	items, err := h.svc.ListAttendanceWorkdaySegmentsByUser(r.Context(), tenantID, userID, query.Get("start_date"), query.Get("end_date"))
	if err != nil {
		h.respondError(w, r, http.StatusInternalServerError, operation, err, "failed to list attendance segments")
		return
	}
