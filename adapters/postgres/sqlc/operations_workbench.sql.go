...
// versions:
//   sqlc v1.30.0
// source: operations_workbench.sql
...
const listOperationsWorkbenchCards = `-- name: ListOperationsWorkbenchCards :many
WITH cards AS (
    SELECT
        la.tenant_id,
...
type ListOperationsWorkbenchCardsRow struct {
	TenantID       uuid.UUID          `db:"tenant_id" json:"tenant_id"`
	...
	RouteSection   string             `db:"route_section" json:"route_section"`
	RouteRecordID  uuid.UUID          `db:"route_record_id" json:"route_record_id"`
	Metadata       []byte             `db:"metadata" json:"metadata"`
}

func (q *Queries) ListOperationsWorkbenchCards(ctx context.Context, arg ListOperationsWorkbenchCardsParams) ([]ListOperationsWorkbenchCardsRow, error) {
	rows, err := q.db.Query(ctx, listOperationsWorkbenchCards,
		arg.Lane,
		arg.Category,
		arg.Severity,
		arg.Search,
	...
	defer rows.Close()
	var items []ListOperationsWorkbenchCardsRow
	for rows.Next() {
		var i ListOperationsWorkbenchCardsRow
		if err := rows.Scan(
			&i.TenantID,
			&i.CardKey,
			&i.Lane,
