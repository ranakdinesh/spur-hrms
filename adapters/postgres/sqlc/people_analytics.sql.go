...
const getPeopleAnalyticsWorkspace = `-- name: GetPeopleAnalyticsWorkspace :one
WITH workforce AS (
    SELECT
...
func (q *Queries) GetPeopleAnalyticsWorkspace(ctx context.Context, tenantID uuid.UUID) ([]byte, error) {
	row := q.db.QueryRow(ctx, getPeopleAnalyticsWorkspace, tenantID)
	var workspace []byte
