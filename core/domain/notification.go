...
func DefaultNotificationMasterInputs(tenantID uuid.UUID) []NotificationMasterInput {
	return []NotificationMasterInput{
		...
		{TenantID: tenantID, Code: NotifLeaveClarify, Name: stringPtr("Leave Clarification"), Description: stringPtr("Leave request clarification messages between employees and approvers."), IsInAppEnabled: true, IsEmailEnabled: true, IsPushEnabled: true, EmailSubjectTemplate: stringPtr("{{tenant_name}} leave request clarification"), EmailTextTemplate: defaultEmailTextTemplate("A leave request has a new clarification message.")},
